...
class EpochwiseExtension(pl.LightningModule):
    def __init__(self, encoder, base_lr=1e-3,lr_sched=None, sched_args=None, weight_decay=0.0, dropout_rate=0.0, log_every_step = False, L=20):
        ...
        # epoch-wise
        self.epochrnn = nn.GRU(self.nFilter + encoder.latent_dim,self.nHidden,1,bidirectional=True,batch_first=True)
    ...
    def forward(self, x, c):
        ...
        # concatenate x and latent encoding  
        x   = x.reshape( [-1, self.timeBins, self.nFilter])
        latent = latent.reshape((self.L*batch_size, 1, self.encoder.latent_dim))
        latent = latent.expand(self.L*batch_size, self.timeBins,self.encoder.latent_dim)
        x = torch.cat((x,latent), dim=2)
        x   = x.reshape( [-1, self.timeBins, self.nFilter + self.encoder.latent_dim])
...
if __name__ == '__main__':
    ...
    net.to(cuda)
    c = torch.randn(3*1,188,129,1).to(cuda)
    x=torch.randn(3*net.L,29,129,1).to(cuda)
    x[1,:,2,:]=1
    output=net(x,c)
