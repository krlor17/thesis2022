...
#%% 
MODEL_PATH = "../runs/tied_rnn/checkpoints/tiedRNN-unfrozen/epoch=81-valKappa=0.8703.ckpt"
...
np.save("labels_tied", cluster.labels_)
plt.rc('font', size=16)
...
max_idx = np.argmax(list(count_labels.values()))
major_label = cluster.labels_[max_idx]

C_major = [nights[(valmember == False)&(cluster.labels_ == major_label)]]
C_other = [nights[(valmember == False)&(cluster.labels_ != major_label)]]
loadedData = load_from_disk()

L=20

sampler_major = get_sampler(C_major, loadedData)
sampler_other = get_sampler(C_other, loadedData)

trainer = pl.Trainer(gpus=1)

res0 = trainer.validate(model, sampler_major)
res1 = trainer.validate(model, sampler_other)



conf0 = conf_matrix(model, sampler_major)
conf1 = conf_matrix(model, sampler_other)
